...
def compute_segment_forces(
    ...
    # Propeller slipstream (PX4 SIH: v_slip = sqrt(2T/(rho·pi·r²))).
    # Differentiability fix: for non-propeller segments (`prop_radius == 0`
    # AND `thrust == 0`) the bare expression has 0/0 inside sqrt, whose
    # gradient NaNs through `jnp.where`. Replace the unsafe argument with a
    # constant 1.0 *before* the sqrt for those segments — forward value
    # remains 0 (mask), gradient is finite.
    has_prop = params.prop_radius > 1e-4
    safe_radius_sq = jnp.where(has_prop, params.prop_radius ** 2, 1.0)
    slip_arg = jnp.where(
        has_prop,
        2.0 * thrust / (rho * jnp.pi * safe_radius_sq),
        1.0,
    )
    v_slip_raw = jnp.sqrt(jnp.maximum(0.0, slip_arg))
    v_slip = jnp.where(has_prop, v_slip_raw, 0.0)
